...
#### Cursor / Antigravity / LM Studio /  Другие клиенты

Большинство клиентов, поддерживающих MCP через `stdio`, следуют той же схеме. Вам нужно указать:
- **Команда (Command)**: `node`
- **Аргументы (Arguments)**: `["/path/to/tool-search-tools-mcp/dist/server.js"]`
- **Переменные окружения (Environment Variables)**: `MCP_CONFIG_PATH` (обязательно, если файл не в корне проекта), `MCP_SEARCH_MODE` (опционально).
...
<details>
<summary>Поддерживает ли этот инструмент все MCP-серверы?</summary>
Да, он работает как прокси и может подключаться к любому серверу, следующему спецификации Model Context Protocol (на базе stdio).
</details>

<details>
<summary>Зачем использовать режим поиска "vector"?</summary>
Векторный (семантический) поиск понимает смысл вашего запроса, а не только точные совпадения ключевых слов. Это полезно, когда вы не знаете точного названия инструмента, но знаете, что он должен делать.
</details>

<details>
<summary>Безопасны ли мои данные при использовании векторного поиска?</summary>
Да, по умолчанию проект использует `transformers.js` с моделью `Xenova/all-MiniLM-L6-v2`, которая работает **локально** на вашем компьютере. Никакие данные не отправляются на внешние API для генерации эмбеддингов.
</details>

<details>
<summary>Можно ли использовать это с Claude Desktop?</summary>
Конечно! См. раздел [Использование](#подключение-к-claude-desktop-или-другим-клиентам) для примеров конфигурации.
</details>

<details>
<summary>Как переключаться между режимами поиска "fuse" и "vector"?</summary>
Вы можете установить переменную окружения `MCP_SEARCH_MODE` в значение `fuse` (нечеткий текстовый поиск) или `vector` (семантический поиск). Для Claude Desktop добавьте это в раздел `env` вашей конфигурации сервера.
</details>

<details>
<summary>Где хранятся эмбеддинги и как их очистить?</summary>
По умолчанию эмбеддинги кэшируются в директории `.cache/embeddings` внутри папки проекта. Вы можете изменить этот путь с помощью переменной окружения `MCP_CACHE_DIR`. 

Кэш создается и используется только в режиме **vector**. Он автоматически перестраивается при изменении определений инструментов, а неиспользуемые или устаревшие записи удаляются. Чтобы принудительно обновить индекс, просто удалите эту директорию.
</details>

<details>
<summary>Как добавить больше MCP-серверов для агрегации?</summary>
Отредактируйте файл `mcp-config.json` и добавьте новые серверы в объект `mcpServers`. Формат идентичен конфигурации Claude Desktop.
</details>

<details>
<summary>Можно ли использовать другую модель эмбеддингов?</summary>
Да, вы можете указать другую модель с [Hugging Face](https://huggingface.co/models?library=transformers.js), используя переменную окружения `MCP_EMBEDDING_MODEL`. Убедитесь, что модель совместима с `transformers.js`.
</details>

## Структура проекта

*   `src/server.ts` — Основной файл MCP-сервера. Точка входа, инициализирующая сервер и внутренние инструменты.
*   `src/cli.ts` — CLI-утилита для тестирования поиска инструментов напрямую из терминала.
